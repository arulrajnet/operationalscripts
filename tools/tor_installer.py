...
def __is_already_installed__(package_name):
    packages = run("yum list installed %s" % package_name)
    import re
    match = re.search("Installed Packages", packages)
    if match:
        return True
    return False
...
def __tor_config__():
    ...
    with cd("/etc/privoxy"):
        uncomment("config", r"forward-socks5t(\s)+\/(\s)+127.0.0.1:9050")
...
def __set_system_proxy__(sysinfo):
    ...
    proxy_string = """http_proxy="http://127.0.0.1:8118"
...
def __setup_renew_ip__():
    # TODO Tor config control port password
    run("wget https://github.com/aaronsw/pytorctl/archive/master.tar.gz --no-check-certificate -O /tmp/pytorctl.tar.gz")
    run("mkdir /tmp/pytorctl")
    run("tar -xzvf /tmp/pytorctl.tar.gz -C /tmp/pytorctl --strip-components=1")
    run("pip install /tmp/pytorctl/")
    run("rm -rf /tmp/pytorctl*")
    put(local_path="../assets/ip_renew.py", remote_path="/opt/", mode=0755)
...
def install():
    ...
    run('curl -s --socks5-hostname localhost:9050 http://curlmyip.com')
